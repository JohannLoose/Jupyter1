...
class Individual:
    ...
    def writeIndividualToFile(self):
        # Filename to write
        filename = r"C:\Users\Johann\Desktop\PythonAusgabe.txt"

        # Open the file with writing permission
        myfile = open(filename, 'w')

       
        myfile.write("Connections:\n")
        for x in self.connections:
            connection = self.connections[x]
            myfile.write("%i:    %i -> %i,     W=%f\n" %(connection.innovationNR, connection.start, connection.end, connection.weight))
        myfile.write("Nodes:\n ")
        for x in self.nodes:
            node = self.nodes[x]
            myfile.write("%i:  %f\n " %(node.nodeNR, node.bias))

        myfile.write("Details on predecessor connections:\n")
        for x in self.nodes:
            node=self.nodes[x]
            myfile.write("Node %i:\n" % node.nodeNR)
            for connection in node.predecessorConnections:
                myfile.write("   %i:    %i -> %i,     W=%f\n" %(connection.innovationNR, connection.start, connection.end, connection.weight))
        # Close the file
        myfile.close()
        ...
        if targetNode.type == "input":
            ...
            for connection in targetNode.predecessorConnections:
                if connection.enabled:
                    if connection.start not in [0,1,2]:
                        ö=1
